...
class ExampleAnalysis(Module):
    ...
    def analyze(self, event):
        ...
        eIndex = []
        mIndex = []  # Keeping track of SL electron and muon index
        ...
        for m, muon in enumerate(muons):
            ...
            if muon.pfRelIso04_all >= 0.15:  # Relative Isolation cut R = 0.4 cone
                continue  #'''This isnt correct'''
            n_muons += 1
            mIndex.append(m)
            if n_muons >= 2:
                self.h_cut_vals.Fill(1.5)  # More than one muon found cut
                return False
        ...
        for e, electron in enumerate(electrons):
            ...
            eIndex.append(e)
            if n_electrons >= 2:
                self.h_cut_vals.Fill(2.5)  # More than one electron found cut
                return False
